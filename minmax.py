...
class Node:
    ...
    def generate_children(self):
        ...
        for bille in mouvement:
            possibilite, alliance = toolbox.voisins_jouables(
                self.plateau, bille[0], self.color)
            ...
            for pos, allie in zip(possibilite, alliance):
                ...
                child = Node(score[1], depth=self.depth - 1, move=bille,
                             parent=self, new_pose=pos, color=self.enemi)
    ...
    def minimax(self, depth, max_player, alpha=-1000, beta=1000):
        ...
        if self.depth > 0:
            print(
                f" le meilleur score possible avec une recherche de profondeur {self.depth} est : {value} avec le pions  {move} vers {pos} pour le joueur {self.color}")
